...
import { ArrowLeft, BadgeCheck, Send, Play, Gavel, TrendingUp, Users } from 'lucide-react';
import { PLAYERS, FLAG, initials, ratingOf, ratingCountOf, valuationOf, fmtMoney, bidStatsOf } from '../data';
import { Stars, RateStars } from '../components/Stars';
...
export default function PlayerDetail() {
  ...
  const placeBid = () => {
    ...
    toast(v >= bid.highest ? `Top bid! $${v}K offer sent to ${p.name} 🏆` : `Offer of $${v}K sent to ${p.name}`);
    setOffer('');
  };
    ...
    <div className="mx-auto max-w-3xl px-5 py-12">
      ...
      <div className="card flex flex-wrap items-center gap-5 p-6">
        ...
        <div className="min-w-[200px] flex-1">
          ...
          <div className="mt-1 text-mute">Football · {p.pos} · Age {p.age} · {p.foot} foot</div>
          <div className="mt-2 flex items-center gap-2">
            <Stars value={ratingOf(p)} size={15} />
            <span className="text-sm font-semibold text-amber">{ratingOf(p)}</span>
            <span className="text-[13px] text-mute">· {ratingCountOf(p)} club reviews</span>
          </div>
        </div>
      ...
      {/* Transfer value & live bidding */}
      <div className="card mt-6 p-6">
        <div className="flex flex-wrap items-center justify-between gap-5">
          <div>
            <div className="flex items-center gap-1.5 text-[13px] text-mute"><Gavel size={14} className="text-primary" /> Asking price</div>
            <div className="font-display text-4xl font-bold grad-text">{fmtMoney(valuationOf(p))}</div>
            <div className="mt-1 text-[12.5px] text-mute">Negotiable · open to offers</div>
          </div>
          <div className="flex gap-6">
            <div className="text-center">
              <div className="flex items-center justify-center gap-1 font-display text-2xl font-bold"><TrendingUp size={16} className="text-emerald-400" />{fmtMoney(bid.highest)}</div>
              <div className="text-[11px] text-mute">Top bid</div>
            </div>
            <div className="text-center">
              <div className="flex items-center justify-center gap-1 font-display text-2xl font-bold text-primary"><Users size={16} />{bid.bidders}</div>
              <div className="text-[11px] text-mute">Clubs bidding</div>
            </div>
          </div>
        </div>
        {isClub ? (
          <div className="mt-5 border-t border-white/10 pt-5">
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2.5">
              ...
              <button onClick={placeBid} className="btn-primary"><Gavel size={15} /> Place bid</button>
              <button onClick={() => setOffer(String(bid.highest + 10))} className="btn-ghost text-[13px]">Outbid (+$10K)</button>
            </div>
            <p className="mt-2.5 text-[12px] text-mute">Bid above or below the asking price — the player can accept, decline or counter.</p>
          </div>
        ) : (
